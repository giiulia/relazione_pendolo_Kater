...
_T23, _T24, _T13, _T14 = symbols('_T23 _T24 _T13 _T14', real=True)
...
#sigma_T = np.sqrt(s_T2[3]**2 + s_T2[4]**2 + s_T1[3]**2 + s_T1[4]**2) formula semplice
sigma_T = np.power(derivata_T23*s_T2[3], 2)+ np.power(derivata_T24*s_T2[4], 2) + np.power(derivata_T13*s_T1[3], 2) + np.power(derivata_T14*s_T1[4], 2) #formula con le derivate
sigma_T = np.array(sigma_T, dtype=np.float64) 
sigma_T = np.sqrt(sigma_T)
print("errore su T* è: {}".format(sigma_T))


_T= symbols('_T', real=True) 
formula_funzione = 4*np.power(np.pi, 2)*0.994/np.power(_T, 2)
...
#----------------------------CORREZIONE dell'approssimazione di angolo infinitesimo
teta = np.arcsin(dati.b/dati.a)
sigma_teta = 0.0008 #rad
print(teta)

_T, _teta= symbols('_T _teta', real=True) 
formula_funzione2 = 4*np.power(np.pi, 2)*0.994*np.power(1+(_teta**2/16), 2)/np.power(_T, 2) #correzione al secondo ordine della funzione di prima
...
sigma_g1 = np.power(derivata_T*sigma_T, 2)+np.power(derivata_teta*sigma_teta, 2)
sigma_g1 = np.array(sigma_g1, dtype=np.float64) 
sigma_g1 = np.sqrt(sigma_g1)
print("l'errore casuale su g1 è: {}".format(sigma_g1))

g_1 = 4*np.power(np.pi, 2)*0.994*((1+(np.power(teta, 2)/16))**2) / (T**2)
g_2 = 4*np.power(np.pi, 2)*0.994*((1+(np.power(teta, 2)/16)+(9*np.power(teta, 4)/1024)+(25*np.power(teta, 6)/16384))**2) / (T**2) #correzione al sesto ordine della funzione di prima
Errore_sist = g_2-g_1 #su g_1
print("l'errore sistematico sulla g nuova è: {}".format(Errore_sist))

sigma_g_tot = sigma_g_1**2 + Errore_sist**2
sigma_g_tot= np.sqrt(sigma_g_tot)
print("accelerazione di gravita NUOVA g1: {}±{}".format(g_1, sigma_g_tot))
